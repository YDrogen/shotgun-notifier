...
- [ ] 1. Project scaffolding + dependencies
  ...
  **Acceptance Criteria**:
  ...
  - [ ] `bun run src/index.ts` executes without errors (placeholder output)
  - [ ] `bunx playwright install chromium` completed, Chromium binary available
  - [ ] `tsc --noEmit` passes with zero errors
  - [ ] `package.json` contains scripts: `start`, `dev`, `check`
  - [ ] `.gitignore` includes `node_modules/`, `config.json`, `.env`
...
- [ ] 2. Config schema (Zod) + types + example config
  ...
  **Acceptance Criteria**:
  ...
  - [ ] `bun test src/config.test.ts` passes (valid, invalid, edge cases)
  - [ ] `loadConfig('config.example.json')` returns valid `Config` object
  - [ ] `loadConfig('nonexistent.json')` throws with clear error message
  - [ ] `loadConfig` with invalid URL throws Zod validation error with field details
  - [ ] `pollIntervalMs` defaults to 60000 when omitted from config
...
- [ ] 3. Pino logger + Discord notifier
  ...
  **Acceptance Criteria**:
  ...
  - [ ] `bun test src/notifier.test.ts` passes (success, rate limit, error cases)
  - [ ] `sendNotification` sends Discord embed with event name, URL, state, categories, timestamp
  - [ ] `sendUnhealthyAlert` sends warning embed with failure count
  - [ ] Discord 429 response is handled gracefully with warning log
  - [ ] Invalid webhook URL returns false from `validateWebhookUrl`
...
- [ ] 4. Playwright validation gate
  ...
  **Recommended Agent Profile**:
  - **Category**: `deep`
  - **Skills**: [`playwright`]
  - Reason: Needs Playwright skill for browser automation patterns and anti-detection knowledge
  ...
  **Pattern References**:
  - `https://playwright.dev/docs/network` — Network interception API
  - `https://playwright.dev/docs/api/class-page#page-event-response` — Response event handler

  **External References**:
  - Playwright network: `https://playwright.dev/docs/network` — Intercept and modify requests
  - Existing research: Shotgun uses Next.js SSR + client hydration
  - `jeffersongt/shotgun-monitor` — Puppeteer approach (outdated selectors, dynamic CSS classes)
  - `swappedphantom-cmd/idf-events` — Playwright approach with partial class matching

  **Acceptance Criteria**:

  - [ ] `VALIDATION-GATE-REPORT.md` exists in project root with documented findings
  - [ ] Report includes: network endpoints found, DOM detection strategy, bot detection status
  - [ ] Script runs without crashing (even if Shotgun blocks headless — document that finding)
  - [ ] At least ONE viable strategy for detecting ticket status is documented
...
- [ ] 5. State tracker
  ...
  **What to do**:
  - Create `src/state.ts` with:
    ...
    - `updateState(url: string, newStatus: EventStatus): StateChange[]` — compare with previous state and return changes
  ...
  **Pattern References**:
  - `src/types.ts:EventState`, `EventStatus`, `TicketCategory` — State model defined in Task 2
  - State machine pattern: transitions between `available`, `sold_out`, `unknown`

  **External References**:
  - No external dependencies needed — pure TypeScript logic

  **Acceptance Criteria**:

  - [ ] `bun test src/state.test.ts` passes all 5+ test cases
  - [ ] First update for any URL returns state change (unknown → detected state)
  - [ ] Consecutive updates with same state return no state change
  - [ ] State transitions correctly detected (sold_out→available, available→sold_out)
  - [ ] Consecutive failure tracking works, `isUnhealthy` returns true at threshold 5
  - [ ] No file I/O — pure in-memory state
...
- [ ] 6. Scraper module
  ...
  **What to do**:
  ...
  - **PRIMARY strategy**: Network interception — use `page.on('response')` to capture API responses containing ticket data
    - Listen for XHR/fetch responses with URLs containing `ticket`, `event`, `availability`, or `shotgun`
    - Use `Promise.all([page.waitForResponse(matcher), page.goto(url)])` to avoid race conditions
    - Parse JSON responses for ticket availability data
  - **FALLBACK strategy**: DOM scraping using semantic selectors (NOT dynamic CSS classes)
    ...
    - Check `__NEXT_DATA__` script tag for server-side rendered state
  ...
  **Recommended Agent Profile**:
  - **Category**: `deep`
  - **Skills**: [`playwright`]
  - Reason: Core Playwright usage patterns, network interception, anti-detection
  ...
  **Pattern References**:
  - `VALIDATION-GATE-REPORT.md` — Documented DOM selectors and API endpoints from Task 4
  - `src/types.ts:EventState`, `TicketCategory`, `ScrapeResult` — Type definitions from Task 2
  - `src/logger.ts` — Pino logger for structured logging

  **API/Type References**:
  - `src/types.ts:EventState` — `'available' | 'sold_out' | 'unknown'` used for scrape results
  - `src/types.ts:TicketCategory` — Category shape expected from scraper output

  **External References**:
  - Playwright API: `https://playwright.dev/docs/api/class-page#page-event-response` — Response event
  - Playwright network: `https://playwright.dev/docs/network` — Request/response interception
  - `page.on('response')` pattern: Capture XHR/fetch responses for API data
  - `page.route()` pattern: Intercept and abort image/CSS/font requests
  ...
  **Acceptance Criteria**:
  ...
  - [ ] Scraper uses selectors from VALIDATION-GATE-REPORT.md (not hardcoded dynamic classes)
...
- [ ] 7. Orchestrator (poll loop + backoff + browser lifecycle)
  ...
  **Pattern References**:
  - `src/types.ts:Config`, `EventState`, `StateChange` — Type definitions
  - `src/state.ts:EventStateTracker` — State tracking with `updateState()` and `isUnhealthy()`
  - `src/scraper.ts:scrapeEvent()` — Scraper interface
  - `src/notifier.ts:sendNotification()`, `sendUnhealthyAlert()` — Notifier interface
  - `src/logger.ts` — Pino logger instance

  **API/Type References**:
  - `src/types.ts:Config` — Config shape with events, pollIntervalMs, discordWebhookUrl
  - `src/types.ts:StateChange` — Change detection result shape
...
- [ ] 8. CLI entry point + graceful shutdown + integration testing
  ...
  **Pattern References**:
  - `src/config.ts:loadConfig()` — Config loading with Zod validation
  - `src/orchestrator.ts:startMonitoring()` — Poll loop entry point
  - `src/notifier.ts:validateWebhookUrl()` — Webhook validation
  - `src/scraper.ts:initBrowser()`, `closeBrowser()` — Browser lifecycle

  **API/Type References**:
  - `src/types.ts:Config` — Full config shape expected by orchestrator
